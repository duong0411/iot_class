...
void Application::Initialize() {
    ...
    // Initialize scheduled audio manager
    ScheduledAudio::GetInstance().Initialize();
...
void Application::Run() {
    ...
    while (true) {
        ...
        if (bits & MAIN_EVENT_SEND_AUDIO) {
            const bool suppress =
                std::chrono::steady_clock::now() < suppress_send_audio_until_;
            while (auto packet = audio_service_.PopPacketFromSendQueue()) {
                if (suppress) {
                    // Drop residual wake / ding / room noise so cloud ASR does not
                    // invent a first turn like "hẹn gặp lại".
                    continue;
                }
        ...
        if (bits & MAIN_EVENT_CLOCK_TICK) {
            ...
            // After wake ding: wait grace, then tell the cloud to start ASR.
            if (defer_start_listening_ && GetDeviceState() == kDeviceStateListening &&
                std::chrono::steady_clock::now() >= suppress_send_audio_until_) {
                defer_start_listening_ = false;
                while (audio_service_.PopPacketFromSendQueue())
                    ;
                if (protocol_) {
                    protocol_->SendStartListening(listening_mode_);
                }
                ESP_LOGI(TAG, "Listening armed: speak now (cloud ASR started)");
            }

            // Check scheduled audio alarm trigger
            ScheduledAudio::GetInstance().CheckSchedule();
...
void Application::HandleNetworkConnectedEvent() {
    ...
    if (!esp_sntp_enabled()) {
        ...
        esp_sntp_init();
        ESP_LOGI(TAG, "SNTP time sync initialized (ICT-7)");
    }
    ...
    // Fetch remote schedule ASAP after Wi-Fi so the next minute match is not missed.
    ScheduledAudio::GetInstance().RefreshRemoteSchedule();
...
void Application::ActivationTask() {
    ...
    // Initialize the protocol (connects on demand when chat/wake starts)
    InitializeProtocol();
...
void Application::InitializeProtocol() {
    ...
    protocol_->OnIncomingAudio([this](std::unique_ptr<AudioStreamPacket> packet) {
        // Match stable firmware: only queue cloud TTS while Speaking.
        // Scheduled/local clips call AudioService::PushPacketToDecodeQueue directly.
...
void Application::HandleToggleChatEvent() {
    ...
    if (state == kDeviceStateActivating) {
        ...
        Alert(Lang::Strings::ERROR, "Chưa kết nối Wi-Fi", "warning", Lang::Sounds::OGG_EXCLAMATION);
...
void Application::ContinueWakeWordInvoke(const std::string& wake_word) {
    ...
    // Do NOT upload the wake-word PCM. On this board (no AEC) that buffer almost
    // always contains room noise; cloud ASR then invents a first user turn
    // (e.g. YouTube outro → LLM replies "Hẹn gặp lại nhé").
    ...
    // Ding first, then open mic after it drains (+ grace below).
    play_popup_on_listening_ = true;
    SetListeningMode(GetDefaultListeningMode());
}
...
void Application::StartListeningAudio() {
    ...
    // Ding first, mic second. Opening the mic while the popup plays (no AEC on
    // this board) ships the ding + ambient into cloud ASR as the first turn —
    // which is why "hi lily" was followed by a random goodbye reply.
    if (play_popup_on_listening_) {
        play_popup_on_listening_ = false;
        pending_listening_start_ = true;
        ConfigureWakeWordForListening();
        audio_service_.PlaySound(Lang::Sounds::OGG_POPUP);
        ESP_LOGI(TAG, "Wake ready: playing ding, mic opens after playback drains");
        return;
    }

    // Discard any packets encoded while we waited for the ding.
    while (audio_service_.PopPacketFromSendQueue())
        ;

    // Warm the mic locally first; only arm cloud ASR after a grace period so
    // ding echo / TV ambient is not the first "user" utterance.
    audio_service_.EnableVoiceProcessing(true);
    ConfigureWakeWordForListening();
    defer_start_listening_ = true;
    suppress_send_audio_until_ =
        std::chrono::steady_clock::now() + std::chrono::milliseconds(1200);
    ESP_LOGI(TAG, "Listening warmup 1.2s after ding — wait, then speak");
}
...
bool Application::PlayScheduleAnnouncement(const std::string& text) {
    if (text.empty() || !protocol_) {
        return false;
    }
    if (GetDeviceState() != kDeviceStateIdle) {
        ESP_LOGW(TAG, "Schedule announcement skipped: state=%d", (int)GetDeviceState());
        return false;
    }

    // Run on the main loop so we never race the state machine / audio channel.
    Schedule([this, text]() {
        ...
        std::string prompt =
            std::string("Hãy đọc to đúng nguyên văn thông báo sau, không chào, không hỏi lại: ") +
            text;
        ESP_LOGI(TAG, "Schedule -> cloud TTS: %s", prompt.c_str());
        protocol_->SendWakeWordDetected(prompt);
        play_popup_on_listening_ = true;
