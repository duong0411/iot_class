...
public:
    ...
    // Schedule/backend announcement via the normal Xiaozhi cloud TTS path (no local MP3).
    bool PlayScheduleAnnouncement(const std::string& text);
...
private:
    ...
    // After wake, drop mic uplink for a short grace so residual / room noise is not ASR'd.
    std::chrono::steady_clock::time_point suppress_send_audio_until_{};
    bool defer_start_listening_ = false;
    int clock_ticks_ = 0;
